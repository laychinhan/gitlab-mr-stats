...
export async function getMergeRequestStats(
  ...
  avgTimeToFirstComment: number | null;
  avgTimeToApproval: number | null;
  avgTimeToMerge: number | null;
  totalMRs: number;
  mergedMRs: number;
}> {
  // Build WHERE clause with date filtering and exclude closed MRs
  let whereClause = 'project_id = ? AND state != "closed"';
  ...
  return {
    avgTimeToFirstComment: stats.avg_time_to_first_comment,
    avgTimeToApproval: stats.avg_time_to_approval,
    avgTimeToMerge: stats.avg_time_to_merge,
    totalMRs: stats.total_mrs,
    mergedMRs: stats.merged_mrs
  };
}
...
export async function countMrsWithLateFirstComment(
  ...
  // Build WHERE clause with date filtering and exclude closed MRs
  let whereClause = 'project_id = ? AND state != "closed" AND first_comment_at IS NOT NULL';
  const params: any[] = [projectId];
